...
Use this code for the V2.1 and V2.2 versions of the emulator PCB - use chained_rtd_emulator for versions V0 anbd V1

Control one or more daisy-chained 74HC595 boards (8 control bits per board).
Wiring (Pi 4):
...
class ChainDriver:
    ...
    Each board contributes one byte; all 8 bits are used (Q0..Q7)
    """
    ...
    def set_board_bit(self, board_idx, bit_idx, value):
        if 0 <= board_idx < self.n_boards and 0 <= bit_idx < 8:
    ...
    def set_board_bits(self, board_idx, bits):  # type: (int, List[int]) -> None
        if 0 <= board_idx < self.n_boards:
            packed = [(1 if (i < len(bits) and bits[i]) else 0) for i in range(8)]
...
class App(tk.Tk):
    ...
    def _rebuild_board_controls(self):
        ...
        for board_index in range(self.driver.n_boards):
            ...
            quick.grid(row=1, column=0, columnspan=8, pady=(2, 4))
            ttk.Button(quick, text="All 0", width=6,
                       command=lambda b=board_index: self._set_board_bits(b, [0,0,0,0,0,0,0,0]))\
                .grid(row=0, column=0, padx=2)
            ttk.Button(quick, text="All 1", width=6,
                       command=lambda b=board_index: self._set_board_bits(b, [1,1,1,1,1,1,1,1]))\
