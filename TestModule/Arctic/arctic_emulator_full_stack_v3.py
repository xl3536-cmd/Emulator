...
- Slider for °C registers
- Bitfield editor for selected registers
- Double-click edit in table

Manual-only register control.
"""
...
PORT_INFO = {
    "Serial File": "/dev/ttyACM0",   # set to your actual link used by backend master
    "Timeout": 1.5,
    "Baudrate": 2400,
    "Bytesize": 8,
    "Parity": "E",
...
REGS: List[Tuple[int, str, str, bool, str, float]] = [
    ...
    (2001, "Working mode", "0=Cooling,1=Floor heating,2=Fan coil heating,5=Hot water", False, "", 1.0),
    (2002, "Cooling temp setpoint", "writable", False, "°C", 1.0),
    (2003, "Heating temp setpoint", "optional/manual", False, "°C", 1.0),
    (2004, "Hot water temp setpoint", "writable", False, "°C", 1.0),
    ...
    (2118, "Compressor operating frequency", "", False, "Hz", 1.0),
    (2119, "Fan motor speed", "", False, "rpm", 1.0),
    (2120, "AC voltage", "", False, "V", 1.0),
    (2121, "AC current", "", False, "A", 1.0),
    (2122, "DC voltage", "", False, "V", 1.0),
    (2123, "DC current", "", False, "A", 1.0),
    ...
    (2136, "Status register 2", "bitfield; bit5=defrost", False, "", 1.0),
...
BITFIELDS: Dict[int, Tuple[str, Dict[int, str]]] = {
    ...
    2135: ("2135 Status register 1 bits", {
        ...
        1: "Compressor status (doc)",
        2: "Compressor status (backend decode bit)",
        3: "Medium wind speed",
        4: "Low wind speed",
        5: "Water pump",
        6: "4-way valve",
...
class DeviceEmulator:
    def __init__(self, unit_id: int, regs: List[Tuple[int, str, str, bool, str, float]]):
        ...
        # Keep the same addressing model your stack already uses.
...
class DeviceTab(ttk.Frame):
    ...
    def __init__(self, parent, device: DeviceEmulator):
        ...
        self.refresh_table()
        self.after(300, self._poll_external_updates)

    def _value_entry_active(self) -> bool:
        return self.focus_get() == self.value_entry
    ...
    def _poll_external_updates(self) -> None:
        try:
            editing_tree_cell = self._edit_entry_cell is not None
            selected = self._selected_addr

            for iid in self.tree.get_children():
                addr = int(iid)
                if editing_tree_cell and selected is not None and addr == selected:
                    continue
                self._update_row(addr)

            # IMPORTANT: don't overwrite user typing in right panel
            if self._selected_addr is not None and not self._value_entry_active():
    ...
    def _update_right_panel(self, addr: int) -> None:
        ...
        # IMPORTANT: do not overwrite typed input
        if not self._value_entry_active():
...
def _init_default_values(device: DeviceEmulator) -> None:
    defaults = {
        2000: 0, 2001: 0, 2002: 20.0, 2003: 35.0, 2004: 45.0,
        2052: 2, 2053: 5, 2054: -1.0, 2056: 0, 2057: 0,

        2100: 26.0, 2102: 25.0, 2103: 24.0, 2104: 5.0, 2105: 3.0,
        2107: -2.0, 2108: 11.0, 2110: 4.0, 2115: 0.0, 2116: 0.0,

        2118: 0, 2119: 650,
        2120: 247, 2121: 0, 2122: 348, 2123: 0,

        2133: 0, 2134: 0, 2135: 0, 2136: 0, 2138: 0,
    }
