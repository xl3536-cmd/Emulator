...
Date: 03/11/2026
Author: Xiaxin Liu
Key update: added realy detector as a gate to ADout.

Valve trigger thresholds are currently:
OPEN when VIN >= 9.0V
CLOSE when VIN is in 1.8V..2.2V
On either trigger, each valve now waits 1.0s before ramp start.
After that delay, ramp starts only if that valve’s relay is ON.
If relay is OFF, ramp does not start (or stops if already ramping) and VOUT is held at current value.
Matching is done by valve number mapping (including swapped valves), not raw channel numbering.
...
RELAY_GATE_DELAY_SEC = 1.0  # Wait before relay-gated ramp start
...
class IOController:
    ...
    def __init__(self):
        ...
        # Channel -> valve number map (uses valve swap overrides).
        self.channel_to_valve = {}
        ...
        for valve_num in range(1, total_valves + 1):
            ...
            if mapped:
                self.channel_to_valve[mapped] = valve_num
        ...
        for stack, num_ch in VIN_BOARDS:
            ...
            for ch in range(1, num_ch + 1):
                self.channel_states[(stack, ch)] = {
                    ...
                    'manual_vout': V_MIN,
                    'pending_cmd': None,      # OPEN or CLOSE waiting for relay-gated start
                    'pending_since': 0.0,
                }
    ...
    def poll_inputs(self, relay_valve_states: Dict[int, bool] = None) -> bool:
        """
        Poll all input channels and update outputs.
        relay_valve_states: optional dict {valve_num: relay_on_bool}, keyed by valve number.
        ...
        relay_gate_enabled = isinstance(relay_valve_states, dict) and len(relay_valve_states) > 0
        ...
        for stack, num_ch in VIN_BOARDS:
            ...
            for ch in range(1, num_ch + 1):
                ...
                valve_num = self.channel_to_valve.get(key)
                relay_on = True
                if relay_gate_enabled:
                    relay_on = bool(relay_valve_states.get(valve_num, False))

                # Manual override path: ignore VIN commands and hold user VOUT.
                ...
                # Queue explicit commands and wait RELAY_GATE_DELAY_SEC before starting ramp.
                if open_cmd and state['mode'] not in ('RAMP_UP', 'HIGH') and state.get('pending_cmd') != 'OPEN':
                    state['pending_cmd'] = 'OPEN'
                    state['pending_since'] = now
                elif close_cmd and state['mode'] not in ('RAMP_DOWN', 'LOW') and state.get('pending_cmd') != 'CLOSE':
                    state['pending_cmd'] = 'CLOSE'
                    state['pending_since'] = now

                pending_cmd = state.get('pending_cmd')
                pending_since = float(state.get('pending_since', 0.0) or 0.0)
                if pending_cmd and (now - pending_since) >= RELAY_GATE_DELAY_SEC:
                    state['pending_cmd'] = None
                    state['pending_since'] = 0.0

                    if (not relay_gate_enabled) or relay_on:
                        activity_detected = True
                        self.last_change_time = now
                        ...
                        if pending_cmd == 'OPEN':
                            state['mode'] = 'RAMP_UP'
                            print(f"[IO] V{valve_num:02d} OPEN cmd ({vin:.2f}V), relay ON, ramping vout UP")
                        else:
                            state['mode'] = 'RAMP_DOWN'
                            print(f"[IO] V{valve_num:02d} CLOSE cmd ({vin:.2f}V), relay ON, ramping vout DOWN")
                    else:
                        state['mode'] = 'HOLD'
                        print(f"[IO] V{valve_num:02d} cmd blocked, relay OFF, holding {state['current_vout']:.2f}V")

                # If relay turns OFF during ramp, pause immediately at current voltage.
                if relay_gate_enabled and (not relay_on) and state['mode'] in ('RAMP_UP', 'RAMP_DOWN'):
                    state['mode'] = 'HOLD'
                    state['pending_cmd'] = None
                    state['pending_since'] = 0.0
                    print(f"[IO] V{valve_num:02d} relay OFF during ramp, holding {state['current_vout']:.2f}V")
                ...
                if state['mode'] in ('RAMP_UP', 'RAMP_DOWN'):
                    ...
                    if state['mode'] == 'RAMP_UP':
                        ...
                        if frac >= 1.0:
                            state['mode'] = 'HIGH'
                            print(f"[IO] V{valve_num:02d} ramp UP complete")
                    else:
                        target = V_MIN
                        new_v = state['ramp_start_v'] + (target - state['ramp_start_v']) * frac
                        if frac >= 1.0:
                            state['mode'] = 'LOW'
                            print(f"[IO] V{valve_num:02d} ramp DOWN complete")
                elif state['mode'] == 'HIGH':
                    new_v = V_MAX
                elif state['mode'] == 'LOW':
                    new_v = V_MIN
                else:
                    # HOLD (or unknown): keep current output where it is.
                    new_v = state['current_vout']
...
class App(tk.Tk):
    ...
    def _poll_io(self):
        ...
        relay_data = self.relay_detector.poll() if self.relay_detector.enabled else None
        relay_valve_states = self._relay_data_to_valve_states(relay_data)

        if self.io_controller.enabled:
            activity = self.io_controller.poll_inputs(relay_valve_states=relay_valve_states)
        ...
        self._update_relay_display(relay_data)
    ...
    def _relay_data_to_valve_states(self, relay_data: Dict[int, Dict[str, Any]]) -> Dict[int, bool]:
        """Convert relay stack/channel data to {valve_num: relay_on} map."""
        valve_states = {}
        if not relay_data:
            return valve_states
        ...
        for board in RELAY_STACK_BOARDS:
            ...
            for ch in range(1, channels + 1):
                ...
                if state is None:
                    continue
                valve_states[valve_num] = bool(int(state))
        return valve_states

    def _update_relay_display(self, data: Dict[int, Dict[str, Any]] = None):
